...
HeadSolver::HeadSolver(const std::string& path){
    this->Init(path);
}
...
void HeadSolver::Init(const std::string& path){
//    this->configPath =
//            static_cast<std::string>(SOURCE_FILE_PATH) + "/config/BasicSolver/HeadSolver/HeadSolver.json";
    ...
    if (!this->robotObj.contains("JointsIndex")) {
         throw std::logic_error("[HeadSolver::Init] Does not contain JointsIndex");
     }
     ...
     try {
         ...
         throw std::logic_error(
             std::string("[HeadSolver::Init] Failed to parse JointsIndex! \n")
             + "Error: " + e.what()
         );
     }
    ...
    if (!this->robotObj.contains("JointsName")) {
        throw std::logic_error("[HeadSolver::Init] Does not contain JointsName");
    }
    ...
    try {
        ...
        throw std::logic_error(
            std::string("[HeadSolver::Init] Failed to parse JointsName! \n")
            + "Error: " + e.what()
        );
    }
    ...
    if (!this->robotObj.contains("Direction")) {
         throw std::logic_error("[HeadSolver::Init] Does not contain Direction");
     }
     ...
     try {
         ...
         throw std::logic_error(
            std::string("[HeadSolver::Init] Failed to parse Direction! \n")
            + "Error: " + e.what()
         );
     }
    ...
    if (!this->robotObj.contains("EulerAxis")) {
        throw std::logic_error("[HeadSolver::Init] Does not contain EulerAxis");
    }

    auto types = JsonParser::JsonArray2StdVecStr(robotObj["EulerAxis"].as_array());

    if(this->jointsName.size() != this->jointsIndex.size()){
        throw std::logic_error("[HeadSolver::Init] Names and indices size mismatch!");
    }
    ...
    if(1){
        std::cout << "[HeadSolver::Init] JointsInfo: " << std::endl;
...
Eigen::Vector3d HeadSolver::Solve(const Eigen::Matrix4d &mat){
    if(!this->initFlag){
        throw std::logic_error("[HeadSolver::Solve] Plz initialize first! ");
    }
...
//std::vector<int> HeadSolver::GetDirection()
//{
...
std::vector<RobotBase::JointInfo> HeadSolver::GetJointsInfo(){
    if(!this->initFlag){
        throw std::logic_error("[HeadSolver::GetJointsInfo] Plz initialize first! ");
    }
