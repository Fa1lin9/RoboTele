...
G1Dof23DualArmSolver::G1Dof23DualArmSolver(const ArmSolver::BasicConfig &config_)
     ...
     relativeTol(config_.relativeTol)
{
    ...
    if(config_.baseFrameName.size() != 1){
        std::string error = " As for G1Dof23DualArmSolver,the size of the baseFrameName should be 1! ";
    ...
    if(config_.baseOffset.size() != 1){
        std::string error = " As for G1Dof23DualArmSolver,the size of the baseOffset should be 1! ";
        throw std::length_error(error);
    }else{
    ...
    if(config_.targetOffset.size() != 2){
        std::string error = " As for G1Dof23DualArmSolver,the size of the targetOffset should be 2! ";
        throw std::length_error(error);
    }else{
        this->targetOffset = config_.targetOffset;
    }
...
G1Dof23DualArmSolver::~G1Dof23DualArmSolver()
{
...
boost::optional<Eigen::VectorXd> G1Dof23DualArmSolver::Solve(
...
std::vector<pinocchio::SE3> G1Dof23DualArmSolver::Forward(const Eigen::VectorXd& q){
    return std::vector<pinocchio::SE3>{};
}

void G1Dof23DualArmSolver::Info(){
    std::cout << " ----------------------------------------------------- "<< std::endl;
